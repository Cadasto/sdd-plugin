...
1. **Re-vendor, if stale.** Compare the plugin's own tool version (`<plugin-root>/tools/sdd-check.py`'s `__version__`) against `check.version`. When they differ, or `check.script` does not exist yet, copy the plugin's copy over `check.script` (creating it if this repository never vendored one), `chmod +x`, and write the new version into `check.version`. When the two already agree, do nothing — a second `--upgrade` run must find nothing to re-vendor.
   ...
   **1b. Add the starter changelog, if missing.** A repository scaffolded before step 3c existed has no `CHANGELOG.md`, and `check.changelog.path` (default `CHANGELOG.md`) names a hard `ERROR` when the file is absent — same check as step 3c. If it does not already exist, create it with exactly a `# Changelog` heading and a blank `## [Unreleased]` section. Never overwrite one that already has content.
...
3. **Add missing `kind:` frontmatter.** The scope is closed: exactly ten files — the six process documents this skill writes (`docs/development-process.md`, `docs/ai-workflow.md`, `docs/ci.md`, the three index READMEs) and the four `_template.md` stubs it leaves in each kind's folder (`docs/requirements/_template.md`, `docs/specifications/_template.md`, `docs/adr/_template.md`, `docs/plans/_template.md`). For any of those ten that opens with no `kind:` line, add the line the current template uses for that file — `kind: requirement`/`specification`/`adr`/`plan` for the four stubs. Nothing wider: a hand-authored requirement, specification, ADR or plan is the maintainer's own content, `kind:` included or not, and `--upgrade` never edits it. Never touch a document outside this list, and never touch a body.
