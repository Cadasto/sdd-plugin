...
1. **Detect existing structure, then route.** Glob for `docs/.sdd.yaml`, `docs/requirements/`, `docs/specifications/`, `AGENTS.md`. When the descriptor exists, Grep it for a line matching `^\s*check:` — the `check:` block the 0.6 descriptor carries and a 0.5.x descriptor lacks. Take the first route that matches:
   - **`--upgrade` given** → run **`--upgrade` — top-up an existing repository** below, instead of steps 2–6.
   - **A descriptor exists with no `check:` block** → this is a 0.5.x repository. Run **`--upgrade` — top-up an existing repository** below, instead of steps 2–6, exactly as if `--upgrade` had been given, and open the report with `no check: block in docs/.sdd.yaml — ran --upgrade`. The generic top-up would leave it without the gate, the markers and the regeneration.
   - **A descriptor exists with a `check:` block** → a top-up run of steps 2–6: only create missing pieces and report what was skipped.
   - **No descriptor** → a fresh run of steps 2–6.
...
Same non-destructive contract as the fresh path, run against a repository scaffolded before the gate existed (0.5.x) or before a later descriptor key was added. It runs when `--upgrade` is given, and without the flag whenever the descriptor has no `check:` block (step 1). Never touch a key that already exists or a document body that already has content — only add what is missing, then report exactly what changed so a maintainer can predict it before running the command. The steps are ordered by dependency: the descriptor gains its `check:` block before anything writes into it.
...
6. **Wire the build target.** If the `spec-check` target does not already run the vendored gate, propose the wiring — the same recipe the fresh path writes at step 5, `python3 <check.script> selftest && python3 <check.script> check` — so `<build_entrypoint> <spec_check_target>` and CI run the vendored gate. This includes a 0.5.x `spec-check` stub that runs something else or nothing. Do not silently rewrite an existing build file; propose the diff, and say in the report that CI does not run the gate until the maintainer applies it.

7. **Regenerate.** Run `python3 <check.script> generate --verify --root .` (when `python3` is unavailable, skip this step and say the regeneration could not run here). `--verify` writes nothing; it prints a diff per stale block and every `refused — …` line the real run would print. It exits 1 on any staleness — expected right after `--upgrade` step 5 wrapped a table — so read the output, not the exit code. A row is matched by the identifier or spec name inside its first cell, so a reformatted cell (bare, backticked, or linked differently) for a record that exists is rewritten, never refused. Each refusal names its cause; act on it, then rerun step 7:
