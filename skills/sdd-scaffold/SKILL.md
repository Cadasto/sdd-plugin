...
5. **Stub the build gate.** If the chosen `build_entrypoint` has no `spec_check_target`/`ci_target`, offer to add stub targets: a `spec-check` target that exits non-zero with the message `spec-check: no checker wired yet`, and a `ci` target that depends on it. A stub that passes is a gate that cannot fail, so the report says the gate is a stub until the repo wires its checker. Do not silently rewrite an existing build file — propose the diff.
