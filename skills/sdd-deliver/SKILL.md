...
5. **Gate each task, per lane.** Follow `agents.task_review`: `on` always, `off` never, `lane` means on for the full lane and off for the maintenance lane. The gate dispatches the reviewers named in `agents.reviewers` with the brief attached, plus one scope test: behaviour that no `REQ` or `SPEC §` states is a finding.
