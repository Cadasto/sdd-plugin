...
7. **The integration gate, then round 0, in the branch.** Run the full gate the plan names — `<build_entrypoint> <ci_target>` and `<build_entrypoint> <spec_check_target>` — on the integrated branch and read the output; per-task verification in step 6 is not the full gate. Then run `/sdd-review` on the branch. Its findings are **round 0** of the ledger. Fix the blockers in the branch before the draft PR opens.
