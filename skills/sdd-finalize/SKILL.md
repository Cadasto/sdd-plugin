...
3. **Inbound-link check — before any delete.** Grep `docs/**` for each candidate's path and basename. If anything under `docs/**` still links to a candidate, stop and print the list. The fix is to rewrite the citation to the PR or the `REQ`, then run this again. Never delete a file that is still cited.
4. **First run in this repo.** If `docs/plans/archive/` exists, add every plan in it and its index file to the same candidate list — everything in an archive directory is by definition finished — run the same inbound-link check over them, delete them, and remove the now-empty directory. If `docs/.sdd.yaml` still declares `paths.plans_archive`, remove that line in the same commit.
5. **Delete.** `git rm` each candidate. Use `git rm` so the deletion is staged with the bump; git keeps the history and the PR head ref stays fetchable.
...
- **Never delete a plan that `docs/**` still links to.**
