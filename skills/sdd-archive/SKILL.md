...
2. **Traceability record** — set the delivered `REQ`'s `implementation` status and mirror the promoted spec `status` in its `traceability.yaml` record in this same commit (`references/sdd-methodology.md` §6; it takes effect when the PR merges); confirm it lists the landed `packages`, `tests`, and `probes` — a record carries no plan axis and no PR pointer, do not add one. Then run `sdd-check generate` so the index rows and the requirement detail file's status lines pick up the change. Run the gate from the repository's vendored copy at `check.script`, with `--root .`. When the repository has not vendored it, route `check` to `/sdd-scaffold --upgrade` first — the plugin's own `tools/sdd-check.py` fails the version pin — while `generate` and `context`, which do not check the pin, may run from the plugin's copy. When `python3` is unavailable, say so and leave the block for the next run rather than hand-editing it.
3. **Confirm the gate is clean** — run `sdd-check check`. Run the gate from the repository's vendored copy at `check.script`, with `--root .`. When the repository has not vendored it, route `check` to `/sdd-scaffold --upgrade` first — the plugin's own `tools/sdd-check.py` fails the version pin — while `generate` and `context`, which do not check the pin, may run from the plugin's copy. Read the summary line and confirm it reports clean before continuing. When `python3` is unavailable, say so and confirm by hand instead.
