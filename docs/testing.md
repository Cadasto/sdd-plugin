...
- **Hook tests** — `bash scripts/hooks-test.sh` runs the three hook scripts against scratch git repositories — the session-start orientation, the edit reminder, the one-shot stop nudge on both hosts' payloads — and needs only bash and git; `./scripts/validate.sh` runs it even when Python is absent, and CI runs it last.
