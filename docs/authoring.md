...
This page is for contributors adding or changing a skill, agent, or Cursor rule in this plugin. It is the detailed companion to [AGENTS.md](../AGENTS.md), which is authoritative, and expands on the *how*. The shipped components are the reference examples.
...
- **Skill (auto-invoked router):** `spec-driven-development`, with an always-on `description` only, routes intent. Keep its body lean.
- **Skill (loop or lifecycle):** each `sdd-*` skill owns one stage or one artefact lifecycle. They carry `argument-hint` + `allowed-tools` so they are both auto-invoked on intent and user-invocable as `/sdd-*`. Bodies are imperative, checklist-driven, and cite `references/` rather than restating rules.
- **Agent:** a context-isolated specialist. Three are report-only (`sdd-traceability-auditor`, `sdd-doc-reviewer`, `sdd-spec-conformance-reviewer`) and one, `sdd-implementer`, mutates within its brief.
  - Declare the grant with **`tools:`** (an allowlist) or **`disallowedTools:`** (a denylist over everything the host offers, MCP servers included), **never** `allowed-tools:`. In an agent that key is silently ignored and the agent inherits *all* tools.
  - Write the `description` in the **prose-summary** form: conditions, 2–4 named trigger scenarios, and a "See *When to invoke* in the agent body" pointer. Put the worked scenarios in a `When to invoke` body section, not an `<example>` block in the frontmatter.
...
This repo may be published. Author all content as **general-purpose, language-agnostic SDD tooling**. Do **not** name specific internal or consumer repositories, reproduce absolute filesystem paths, or embed organisation-private project details. Ground claims in the public methodology lineage (Spec Kit, Kiro, the Thoughtworks rigour ladder, RFC-2119); see the Sources section of `references/sdd-methodology.md`.

## Dual-host parity

Skills, agents, and rules are shared by both hosts. The **Cursor** manifest (`.cursor-plugin/plugin.json`) declares each component path; **Claude** discovers the default folders automatically. Keep the two manifests' `name`/`version`/`description`/`author`/`license`/`repository`/`keywords` identical (`scripts/validate.py` checks parity), and keep the Cursor hook commands **workspace-relative** (`bash hooks/session-start.sh`), never `${CLAUDE_PLUGIN_ROOT}` (a Claude-Code-only variable).

## Before committing

Run `./scripts/validate.sh` and `claude plugin validate .`, then test triggering locally; see [testing.md](testing.md). When adding or renaming a component, sync **AGENTS.md**, **README.md**, **CHANGELOG.md**, the Cursor rule **`rules/sdd-context.mdc`**, and the `/sdd-*` list in **`hooks/session-start.sh`** in lockstep.

Then run the **disclosure grep**: search the working tree, `CHANGELOG.md`, and this branch's commit messages for internal or consumer repository names, absolute local paths, organisation-private project details, and private issue references. This repository may be published, so the check covers the commit history too, not just the files.
