...
7. **Run `sdd-check generate` before the first `sdd-check check`.** A table just wrapped in markers,
   or a document `--upgrade` just emitted, holds placeholder content that doesn't yet match what
   `generate` would write from the map. Running `check` first fails the `generated` family on that
   mismatch, every time. `generate` populates every marker block and each requirement's `status`/
   `implementation` frontmatter from the map; only then does `check` have something real to verify.
...
9. **An RFC-2119 keyword outside `docs/specifications/` is now linted.** `MUST`, `SHOULD`, `MAY`,
   ...
   Reword the sentence, move the normative statement into the spec it belongs in, or waive the file
   for this one family: `<!-- sdd-check: allow rfc2119 -->`.
