...
sdd:
  ...
  upstream: ""                      # repo this one consumes via /sdd-gap (blank if none)
...
| Field | Meaning |
|---|---|
| `req_style` | `area-prefixed` (`REQ-AUTH-001`) or `flat-numeric` (`REQ-050`). Drives how `/sdd-requirement` assigns the next ID. |
...
| `build_entrypoint` / `ci_target` / `spec_check_target` | The build tool and the target names `/sdd-verify` and `/sdd-trace` invoke. |
| `use_probes` / `use_strands` | Toggle the optional `PROBE`/`STRAND` machinery. |
| `upstream` | The dependency `/sdd-gap` files spec drafts against (if this repo consumes a sibling SDD repo). |
...
`/sdd-trace` reports these in-session; `/sdd-verify` runs the real `spec_check_target` as part of the Definition of Done.
